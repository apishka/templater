...
class Apishka_Templater_Tests_ExpressionParserTest extends PHPUnit_Framework_TestCase
{
    ...
    /**
     * @dataProvider getTestsForLink
     */
    public function testLinkExpression($template, $expected)
    {
        $env = new Apishka_Templater_Environment($this->getMock('Apishka_Templater_LoaderInterface'), array('cache' => false, 'autoescape' => false, 'optimizations' => 0));
        $stream = $env->tokenize($template, 'index');
        $parser = new Apishka_Templater_Parser($env);
    ...
    public function getTestsForLink()
    {
        $tests = array();
    ...
    /**
     * @expectedException        Apishka_Templater_Error_Syntax
     * @expectedExceptionMessage The "action" function takes at least one argument (the variable and the attributes) in "index" at line 1.
     */
    public function testActionNoArgs()
    {
        $env = new Apishka_Templater_Environment($this->getMock('Apishka_Templater_LoaderInterface'), array('cache' => false, 'autoescape' => false));
        $parser = new Apishka_Templater_Parser($env);

        $parser->parse($env->tokenize('{{ action() }}', 'index'));
    }

    /**
     * @expectedException        Apishka_Templater_Error_Syntax
     * @expectedExceptionMessage The "link" function takes at least one argument (the variable and the attributes) in "index" at line 1.
     */
    public function testLinkNoArgs()
    {
        $env = new Apishka_Templater_Environment($this->getMock('Apishka_Templater_LoaderInterface'), array('cache' => false, 'autoescape' => false));
        $parser = new Apishka_Templater_Parser($env);

        $parser->parse($env->tokenize('{{ link() }}', 'index'));
    }
}
