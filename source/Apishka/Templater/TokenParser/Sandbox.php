...
class Apishka_Templater_TokenParser_Sandbox extends Apishka_Templater_TokenParserAbstract
{
    public function parse(Apishka_Templater_Token $token)
    {
        ...
        if (!$body instanceof Apishka_Templater_Node_Include) {
            foreach ($body as $node) {
                if ($node instanceof Apishka_Templater_Node_Text && ctype_space($node->getAttribute('data'))) {
                    continue;
                }

                if (!$node instanceof Apishka_Templater_Node_Include) {
                    throw new Apishka_Templater_Error_Syntax('Only "include" tags are allowed within a "sandbox" section.', $node->getLine(), $this->parser->getFilename());
                }
            }
        }
