        ...
        while ($this->parser->getStream()->nextIf(Apishka_Templater_Token::PUNCTUATION_TYPE, '?')) {
            if (!$this->parser->getStream()->nextIf(Apishka_Templater_Token::PUNCTUATION_TYPE, ':')) {
                ...
                if ($this->parser->getStream()->nextIf(Apishka_Templater_Token::PUNCTUATION_TYPE, ':')) {
                    ...
                    $expr3 = new Apishka_Templater_Node_Expression_Constant('', $this->parser->getCurrentToken()->getLine());
                }
            } else {
                $expr2 = $expr;
                $expr3 = $this->parseExpression();
            ...
            $expr = new Apishka_Templater_Node_Expression_Conditional($expr, $expr2, $expr3, $this->parser->getCurrentToken()->getLine());
        }
    ...
    public function getFunctionNode($name, $line)
    {
        switch ($name) {
            case 'parent':
                ...
                return new Apishka_Templater_Node_Expression_Parent($this->parser->peekBlockStack(), $line);
            case 'block':
    ...
    public function parseArguments($namedArguments = false, $definition = false)
    {
        ...
        while (!$stream->test(Apishka_Templater_Token::PUNCTUATION_TYPE, ')')) {
            ...
            if ($definition) {
                ...
                $value = new Apishka_Templater_Node_Expression_Name($token->getValue(), $this->parser->getCurrentToken()->getLine());
            } else {
                $value = $this->parseExpression();
            }
            ...
            if ($definition) {
                if (null === $name) {
                    $name = $value->getAttribute('name');
                    $value = new Apishka_Templater_Node_Expression_Constant(null, $this->parser->getCurrentToken()->getLine());
                }
                $args[$name] = $value;
            } else {
                if (null === $name) {
